...
var (
	NO_SCRIPTION_WS    = "wss://report-worker-ng.noscription.org"
	POST_SCRIPTION_URL = "https://api-worker.noscription.org/inscribe/postEvent"
	// TODO: add connection pool
	WebsocketConnectCount    = 3
...
func main() {
	...
	if os.Getenv("NO_SCRIPTION_WS") != "" {
		NO_SCRIPTION_WS = os.Getenv("NO_SCRIPTION_WS")
	}
	if os.Getenv("HTTP_PROXY") != "" {
		tlsclient.SetProxy(os.Getenv("HTTP_PROXY"))
	}
	if os.Getenv("POST_SCRIPTION_URL") != "" {
		POST_SCRIPTION_URL = os.Getenv("POST_SCRIPTION_URL")
	}
	if os.Getenv("MAX_ADD_TIME") != "" {
		MAX_ADD_TIME, err = strconv.Atoi(os.Getenv("MAX_ADD_TIME"))
		if err != nil {
			fmt.Println("MAX_ADD_TIME is not int")
			return
		}
	}
	PROXIES = GetProxies()
	...
	// Print Config
	fmt.Println("POST_SCRIPTION_URL: ", POST_SCRIPTION_URL)
	fmt.Println("NO_SCRIPTION_WS: ", NO_SCRIPTION_WS)
	fmt.Println("XGORGON_URL: ", XGORGON_URL)
	fmt.Println("LARK_WEBHOOK: ", LARK_WEBHOOK)
	fmt.Println("ARB_WEBSOCKET: ", ARBWebSocket)
	fmt.Println("HTTP_PROXY: ", os.Getenv("HTTP_PROXY"))
	fmt.Println("PROXYS Count: ", len(PROXIES))
	fmt.Println("MAX_ADD_TIME: ", MAX_ADD_TIME)
	//
...
func GetLastEventID() {
	...
	conn, response, err := websocket.DefaultDialer.DialContext(ctx, NO_SCRIPTION_WS, PublicHeaders)
	if err != nil {
